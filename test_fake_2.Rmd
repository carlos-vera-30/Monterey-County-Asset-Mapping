...
if (!is.null(asset_data)) {
  ...
  # Remove rows that are just category headers or blanks
  asset_data <- asset_data %>%
    filter(!is.na(Agency))
...
Monterey_County_Assets$color <- as.character(
  sapply(Monterey_County_Assets$Asset_Category, get_category_color)
)
    ...
    <div class="col-md-4">
      <div class="sidebar">
        ...
        <div class="control-group">
          ...
          <small style="color: #666; display: block; margin-top: 5px; margin-bottom: 10px;">For best results, include city and state (e.g., Salinas, CA)</small>
        </div>
...
document.addEventListener("DOMContentLoaded", function() {
  const assets = JSON.parse(`r assets_json`);
  const categoryColors = JSON.parse(`r category_colors_json`);
...
assets_json <- toJSON(Monterey_County_Assets, dataframe = "rows", auto_unbox = TRUE)
categories <- sort(unique(Monterey_County_Assets$Asset_Category))
category_colors_json <- toJSON(as.list(category_colors), auto_unbox = TRUE)

# Create JavaScript variables for global access
js_data_script <- sprintf("
<script>
// Make data available globally for JavaScript functions
window.assets = %s;
window.categoryColors = %s;
console.log('Data loaded from R:', {
  assetsCount: window.assets ? window.assets.length : 0,
  categoriesCount: window.categoryColors ? Object.keys(window.categoryColors).length : 0,
  sampleAsset: window.assets && window.assets.length > 0 ? window.assets[0] : null,
  sampleColors: window.categoryColors ? Object.keys(window.categoryColors).slice(0, 5) : []
});
</script>
...
# Create the leaflet map using the original dataframe
leaflet(data = Monterey_County_Assets) %>%  # Changed from assets_json to Monterey_County_Assets
  addTiles() %>%
  ...
  addCircleMarkers(
    ...
    opacity = 1,
    fillOpacity = 0.8,
    layerId = ~paste0("marker_", seq_len(nrow(Monterey_County_Assets)))  # Changed from asset_data to Monterey_County_Assets
  ) %>%
    ...
    function(el, x) {
      ...
      setTimeout(function() { 
        if (typeof initializeInterface === 'function') {
          ...
          console.error('initializeInterface function not found');
        }
      }, 500);
    }
  ")
...
function initializeInterface() {
  ...
  try {
    // CRITICAL FIX: Get data from the global variables created by R
    // These should be available from the R chunk that creates assets_json and category_colors_json
    if (typeof assets !== 'undefined') {
      allAssets = assets; // This comes from the global 'assets' variable in your HTML
      console.log('Assets loaded from global variable:', allAssets.length);
    }
    
    if (typeof categoryColors !== 'undefined') {
      RCategoryColors = categoryColors; // This comes from the global 'categoryColors' variable
      console.log('Category colors loaded:', Object.keys(RCategoryColors).length);
    }
    
    // Fallback: If the above doesn't work, ensure we have some color mapping
    if (!RCategoryColors || Object.keys(RCategoryColors).length === 0) {
      console.warn('No category colors found, creating default mapping');
      createDefaultColorMapping();
    }

    if (!allAssets || allAssets.length === 0) {
      console.error('No assets loaded from R.');
      showNotification('No asset data loaded. Map may not function correctly.', 'error');
      return;
    }

    // CRITICAL FIX: Ensure each asset has a color property and it's preserved
    allAssets.forEach((asset, index) => {
      if (!asset.color || asset.color === '#000000' || asset.color === 'black') {
        asset.color = getAssetColor(asset.Asset_Category);
        console.log(`Asset ${index} (${asset.Asset_Name}) assigned color: ${asset.color} for category: ${asset.Asset_Category}`);
      }
    });

    // Deep copy to preserve colors
    filteredAssets = allAssets.map(asset => ({...asset}));
    ...
    console.log('Sample asset colors:', allAssets.slice(0, 3).map(a => ({name: a.Asset_Name, color: a.color, category: a.Asset_Category})));
  } catch (error) {
...
function createDefaultColorMapping() {
  // Create a default color mapping if none exists
  const defaultColors = [
  ...
  console.log('Created default color mapping for', categories.length, 'categories');
}
...
function updateMapMarkers() {
  ...
  console.log('Adding', filteredAssets.length, 'markers to map');
  ...
  filteredAssets.forEach((asset, index) => {
    if (typeof asset.Latitude !== 'undefined' && typeof asset.Longitude !== 'undefined' && 
      ...
      // CRITICAL FIX: Always get fresh color and ensure it's not black
      let markerColor = asset.color || getAssetColor(asset.Asset_Category);
      
      // Double-check that color is valid
      if (!markerColor || markerColor === '#000000' || markerColor === 'black' || markerColor === '') {
        markerColor = getAssetColor(asset.Asset_Category);
        asset.color = markerColor; // Update the asset's color property
      }
      
      console.log(`Marker ${index}: ${asset.Asset_Name} - Color: ${markerColor} - Category: ${asset.Asset_Category}`);
      ...
      if (asset.Website) {
        ...
        popupContent += `<a href="${website}" target="_blank">Website</a><br>`;
      }
      ...
      L.circleMarker([asset.Latitude, asset.Longitude], {
        radius: 8,
        fillColor: markerColor,  // Use the verified color
        color: 'white',
        weight: 2,
        opacity: 1,
        fillOpacity: 0.8
...
function getAssetColor(category) {
  // CRITICAL FIX: More robust color retrieval with fallbacks
  console.log(`Getting color for category: "${category}"`);
  console.log('Available categories in RCategoryColors:', Object.keys(RCategoryColors));
  
  if (!category) {
    console.warn('No category provided, using default color');
    return "#607D8B";
  }
  ...
  // If still no color, assign a default and log warning
  if (!color || color === '#000000' || color === 'black') {
    console.warn(`No color found for category "${category}", using default`);
    color = "#607D8B";
  }
...
function updateLegend() {
  ...
  console.log('Updating legend for categories:', displayedCategories);
  ...
  displayedCategories.forEach(category => {
    ...
    const item = document.createElement('div');
    item.className = 'legend-item';
    item.innerHTML = `<div class="legend-color" style="display:inline-block; width:15px; height:15px; background-color: ${color}; margin-right:8px; vertical-align:middle; border-radius: 50%; border: 2px solid white; box-shadow: 0 0 3px rgba(0,0,0,0.3);"></div><span>${category}</span>`;
    legendDiv.appendChild(item);
  });
}

// CRITICAL FIX: Preserve colors during filtering
function applyFilters() {
  ...
  if (selectedCategories.length === 0) {
    ...
    // Deep copy to preserve color properties
    filteredAssets = allAssets
      .filter(asset => selectedCategories.includes(asset.Asset_Category))
      .map(asset => {
        // Ensure color is preserved/refreshed
        const assetCopy = {...asset};
        if (!assetCopy.color || assetCopy.color === '#000000' || assetCopy.color === 'black') {
          assetCopy.color = getAssetColor(assetCopy.Asset_Category);
        }
        return assetCopy;
      });
  }

  console.log('Filtered assets:', filteredAssets.length, 'out of', allAssets.length);
...
function updateMapMarkers() {
  ...
  filteredAssets.forEach((asset) => {
    if (typeof asset.Latitude !== 'undefined' && typeof asset.Longitude !== 'undefined') {
      ...
      L.circleMarker([asset.Latitude, asset.Longitude], {
        radius: 8,
        fillColor: markerColor,  // Use the proper color
        color: 'white',
        weight: 2,
        opacity: 1,
        fillOpacity: 0.8
...
function getAssetColor(category) {
  // Return the color from the mapping, or default color
