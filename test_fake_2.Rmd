...
if (!is.null(asset_data)) {
  ...
  sampled_data <- cleaned_data %>%#remove after no longer needed 
  group_by(Category) %>%
  slice(1) %>%
  ungroup()
...
function initializeInterface() {
  ...
  try {
    // Get data from the global variables created by R
    if (typeof assets !== 'undefined') {
      allAssets = assets;
      console.log('Assets loaded from global variable:', allAssets.length);
    }
    
    // Handle category colors from multiple possible sources
    if (typeof window.categoryColors !== 'undefined') {
      categoryColors = window.categoryColors;
    } else if (typeof categoryColorsFromR !== 'undefined') {
      categoryColors = categoryColorsFromR;
    } else if (typeof window.category_colors !== 'undefined') {
      categoryColors = window.category_colors;
    }
    
    // Fallback: If no data, create defaults
...
function setupEventListeners() {
  ...
  // Distance filter event listener
  const distanceFilter = document.getElementById('distanceFilter');
  if (distanceFilter) {
    distanceFilter.addEventListener('change', updateDistanceFilter);
  }

  // Button event listeners - make sure buttons exist before adding listeners
  ...
  try {
    if ($.fn.DataTable && $('#assetsDataTable').length > 0) {
      ...
      window.assetsDataTable = $('#assetsDataTable').DataTable({
        ...
        order: [[0, 'asc']],
        columnDefs: [
          { targets: [5], orderable: false } // Website column not sortable
        ]
      });
    }
  } catch (error) {
...
function populateCategoryFilter() {
  ...
  categories.forEach(category => {
    ...
    label.htmlFor = id;
    label.style.cssText = 'cursor: pointer; display: flex; align-items: center; font-size: 14px;';
...
function createDefaultColorMapping() {
  const defaultColors = [
    ...
    "#FFD600", "#F8BBD0", "#1976D2", "#388E3C", "#81C784", "#64B5F6"
  ];
...
function updateLegend() {
  ...
  if (displayedCategories.length === 0) {
    ...
    noDataMsg.style.cssText = 'font-style: italic; color: #666; margin: 10px 0;';
    noDataMsg.textContent = 'No categories selected.';
    legendDiv.appendChild(noDataMsg);
    return;
  }
  ...
  displayedCategories.forEach(category => {
    ...
    item.style.cssText = 'margin-bottom: 5px; display: flex; align-items: center;';
    ...
    const textSpan = document.createElement('span');
    textSpan.textContent = category;
    textSpan.style.cssText = 'font-size: 12px; line-height: 1.2;';
