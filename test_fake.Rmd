...
<select id="categoryFilter" multiple style="width: 100%; height: 150px;">
</select>
<div style="margin-top: 5px;">
<button class="btn btn-secondary btn-sm" onclick="selectAllCategories()">Select All</button>
<button class="btn btn-secondary btn-sm" onclick="clearAllCategories()">Clear All</button>
</div>
</div>
...
<div class="control-group">
  ...
  <small style="color: #666;">For best results, include city and state (e.g., Salinas, CA)</small>
  <div class="address-search">
    <button class="btn btn-primary" onclick="searchByAddress()">Search</button>
    <button class="btn btn-info" onclick="getCurrentLocation()">Use Current Location</button>
  </div>
</div>
...
if ($.fn.DataTable.isDataTable('#assetsDataTable')) {
            ...
            console.error("DataTable #assetsDataTable could not be initialized.");
        }
    }, 1000);
}
...
showNotification('Error initializing the map interface. Please check console.', 'error', 10000);
}
}
...
if (!select) return;
const categories = [...new Set(allAssets.map(asset => String(asset.Asset_Category || '')))].sort();
...
showNotification('Error searching for address. Please check network or try again.', 'error');
}
}
...
const displayedCategories = [...new Set(filteredAssets.map(asset => asset.Asset_Category))].sort();

legendDiv.innerHTML = '';
if (displayedCategories.length === 0) {
legendDiv.innerHTML = '<p style="font-style: italic; color: #666;">No categories selected or match current filters.</p>';
return;
}
displayedCategories.forEach(category => {
  ...
  item.innerHTML = `<div class="legend-color" style="background-color: ${color};"></div><span>${category}</span>`;
...
if (typeof window.assetsDataTable === 'undefined' || !window.assetsDataTable) {
  // Try to re-initialize if it was missed.
  if ($.fn.DataTable.isDataTable('#assetsDataTable')) {
    window.assetsDataTable = $('#assetsDataTable').DataTable();
  } else {
...
window.assetsDataTable.clear(); // Clear existing data

const tableData = filteredAssets.map(asset => {
  let distanceText = "N/A"; // Default for the distance column
  ...
  return [
    ...
    distanceText // This is the 7th column (index 6)
  ];
});
...
document.getElementById('distanceFilterDiv').style.display = 'none'; // Hide distance filter
document.getElementById('distanceFilter').value = "5"; // Reset to default distance

selectAllCategories(); // This will call applyFilters()
...
if (window.assetMap) {
  window.assetMap.setView([36.6777, -121.6555], 10); // Reset to initial view (e.g., Salinas)
}

// applyFilters is called by selectAllCategories, which updates map, legend, table
...
// Initial call to set up the interface (moved to onRender of the map for timing)
