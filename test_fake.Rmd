...
// Let's also verify the initialization of the map and data
...
if (typeof window.categoryColors !== 'undefined') {
  ...
  .map(asset => ({ ...asset })); // preserve color
...
function clearAllCategories() {
 ...
 function applyFilters() {
    ...
    if (selectedCategories.length === 0) {
        filteredAssets = [];
        console.log('No categories selected, cleared all assets'); // Debug log
    } else {
        filteredAssets = allAssets.filter(asset =>
            selectedCategories.includes(asset.Asset_Category)
        );
        console.log('Filtered assets count:', filteredAssets.length); // Debug log
    }

    // Apply distance filter if search location exists
    if (searchCoords) {
        applyDistanceFilter();
    }

    // Update map markers with filtered assets
    updateMapMarkers();
    
    // Update legend and table
    updateLegend();
    updateResultsTable();
}
...
function applyDistanceFilter() {
    ...
    const originalLength = filteredAssets.length;
    ...
    const foundCount = filteredAssets.length;
    showNotification(
        `Found ${foundCount} resources within ${maxDistance} miles${maxDistance === 999 ? ' (showing all)' : ''}`,
        'info'
    );
}
...
function searchByAddress() {
    ...
    // Add county and state if not provided
    const fullAddressQuery = address.includes(',') ? 
        address : 
        `${address}, Monterey County, CA`;

    fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(fullAddressQuery)}&limit=1`)
        .then(response => {
        ...
        .then(data => {
            if (data && data.length > 0) {
                ...
                showNotification('Address found! Updating nearby resources...', 'success');
            } else {
...
function setSearchLocation(coords) {
    ...
    if (window.assetMap) {
        ...
        // Center map and zoom based on selected distance
        const distance = parseFloat(document.getElementById('distanceFilter').value);
        let zoom = distance <= 1 ? 14 :
                  distance <= 2 ? 13 :
                  distance <= 5 ? 12 :
                  distance <= 10 ? 11 : 10;
        
        window.assetMap.setView([coords.lat, coords.lng], zoom);
    }
...
function updateMapMarkers() {
    console.log('Updating map markers...'); // Debug log
    
    if (!window.assetMap) {
        console.error('Map not initialized'); // Debug log
        return;
    }
    ...
    console.log('Adding', filteredAssets.length, 'markers to map'); // Debug log

    // Add new markers for filtered assets
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (typeof window.assetMap === 'undefined') {
